                ...
                {step === 7 && (
                  ...
                  >
                    ...
                    <div className="grid grid-cols-2 gap-5 w-full max-w-2xl mx-auto">
                      {[
                        { icon: "📏", title: "长度截断", desc: "无限累加的历史极其容易撞上大模型物理边界导致 API 直接报界限溢出错误。" },
                        { icon: "🕳️", title: "迷失与注意力稀释", desc: "由于无关废话不断稀释核心 Attention 权重，模型极易产生遗忘早期设定的现象（Lost in the middle）。" },
                        { icon: "🗑️", title: "上下文污染", desc: "前面的闲聊噪音、甚至废弃的纠错指令被一并代入，杂音干扰导致 AI 逐渐变笨。" },
                        { icon: "💸", title: "成本与延迟爆炸", desc: "大模型是严格按绝对 Token 计费与推演的，越问到底部越离谱的昂贵与缓慢！" }
                      ].map((item, i) => (
                        <motion.div
                          key={i}
                          ...
                          className="bg-error/5 border border-error/10 hover:border-error/30 p-6 rounded-2xl transition-colors backdrop-blur-sm shadow-xl"
                        >
                  ...
                  >
                    <div className="flex flex-col gap-2 w-full max-w-md">
                      ...
                      >
                        ...
                        <span className="text-white/40 text-[13px] italic mt-auto line-clamp-2">「之前疯狂发散的长篇大论... 带有误导的废弃尝试... 我们不需要这些了」</span>
                ...
                {step === 10 && (
                  ...
                  >
                    <div className="grid grid-cols-2 gap-4 w-full h-[85%]">
                      ...
                      {/* RAG DB */}
                      <div className="flex flex-col justify-center gap-2 bg-[#161616] border border-white/5 p-5 rounded-2xl hover:border-white/10 transition-colors">
                      ...
                      {/* Agent Skills */}
                      <div className="col-span-2 flex flex-col justify-center gap-3 bg-[#111] border border-secondary/20 p-5 rounded-2xl shadow-[0_0_15px_rgba(var(--color-secondary),0.05)] relative overflow-hidden group hover:border-secondary/50 transition-colors">
